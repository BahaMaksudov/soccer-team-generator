...
// ✅ Next.js 15.5: params is typed as a Promise in PageProps
type Params = Promise<{ id: string }>;

export default async function PrintPage({ params }: { params: Params }) {
  const { id } = await params;

  const gen = await prisma.teamGeneration.findUnique({ where: { id } });
  if (!gen) return <div className="p-6">Not found.</div>;

  const teams = JSON.parse(gen.teamsJson) as Array<{
    teamNumber: number;
