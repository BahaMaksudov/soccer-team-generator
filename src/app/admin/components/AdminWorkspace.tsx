...
import TelegramUserLinks from "./TelegramUserLinks";
import TelegramPollImport from "./TelegramPollImport";
import PlayerSelection from "./PlayerSelection";
import GenerationControls from "./GenerationControls";
import TeamPreview from "./TeamPreview";
import DeletePublishedTeams from "./DeletePublishedTeams";

/**
 * Phase 2D.6C — relocated verbatim from src/app/admin/page.tsx (only
 * the relative import paths changed, one level deeper: `./types` ->
 * `../types`, `./components/X` -> `./X`). Zero behavior change.
 *
 * This is the real, still-fully-operational generator application —
 * moved here, not modified, and NOT YET rendered from anywhere. Bare
 * `/admin` now hosts the tenant-entry resolver instead (see
 * src/app/admin/page.tsx and src/app/admin/adminEntry.ts). This
 * component still calls the old flat `/api/admin/*` routes (which
 * still resolve tenancy via the old zero-argument
 * `requireTenantContext()`) — wiring it up to the new canonical,
 * URL-bound tenant routes/APIs is explicitly deferred to a later
 * phase (2D.6D+), not this one. Rendering it from the canonical page
 * before its data calls are also tenant-bound would recreate exactly
 * the stale/mismatched-tenant risk this whole effort exists to
 * prevent (Phase 2D.6C §17).
 *
 * Original orchestration-only design note preserved below:
 *
