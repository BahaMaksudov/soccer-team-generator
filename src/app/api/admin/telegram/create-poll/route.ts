...
export async function POST(req: Request) {
  try {
    ...
    const chatIdStr = String(body.chatId ?? "").trim();     // Telegram chatId (string from UI)
    const pollDateStr = String(body.pollDate ?? "").trim(); // YYYY-MM-DD from <input type="date">
    const customQuestion = body.question ? String(body.question) : "";
    ...
    if (!pollDateStr) {
      return NextResponse.json(
        { error: "pollDate is required (YYYY-MM-DD)" },
        { status: 400 }
      );
    }
